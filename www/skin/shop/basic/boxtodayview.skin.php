...
<aside id="stv">
    <div style="width:100%; border-bottom:1px solid #eeeeee;">
		...
		<style>
			.up_btn{border:0; background:transparent;}
			.down_btn{border:0; background:transparent;}
		</style>
		<span style="position:absolute; top:50%; left:20px;" id="left_btn"></span>
		<span style="position:absolute; top:50%; right:20px;" id="right_btn"></span>
        <?php
        $tv_tot_count = 0;
        $k = 0;
        for ($i=1;$i<=$tv_idx;$i++)
        ...
        $(function() {
            ...
            if (itemQty > itemShow)
            {
				...
				*/
				$('#left_btn').append('<button type="button" id="up" class="up_btn"> < </button>');
				$('#right_btn').append('<button type="button" id="down" class="down_btn"> > </button>');
		...
		<center>
        <p style="margin-bottom:5px;">없음</p>
		</center>
